...
int main(int argc,char* argv[]) {
    ...
    if(rank == 0) {
        ...
        fprintf(outfile, "MPI %d process time (MPI_Send/MPI_Recv): %f  Calculation time: %f  Communication Time: %f\n", number_of_processes, total_time, calc_time, comm_time);
        fclose(outfile); 

        /* Print execution time to screen*/
        printf("MPI %d process time (MPI_Send/MPI_Recv): %f  Calculation time: %f  Communication Time: %f\n", number_of_processes, total_time, calc_time, comm_time);
